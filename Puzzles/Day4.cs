...
public class Day4
{
    ...
    public void Sum_of_pairs_with_invalid_sections_from_puzzle_input(int correctAnswer)
    {
        ...
        var sectionPairs = SectionPairsFromRangeCsv(input);
    ...
    private static List<Tuple<IEnumerable<int>, IEnumerable<int>>> SectionPairsFromRangeCsv(IEnumerable<string> input)
    {
    ...
    [TestCase(798)]
    public void Sum_of_pairs_with_overlapping_sections_from_puzzle_input(int correctAnswer)
    {
        var input = PuzzleInput.GetFile("day4.txt");
        var sectionPairs = SectionPairsFromRangeCsv(input);
        ...
        foreach (var section in sectionPairs)
        {
            collisionSum += CleanupSection.ValidateIsolated(section.Item1.ToArray(),section.Item2.ToArray()) ? 0 : 1;
        }

        Assert.That(collisionSum, Is.EqualTo(correctAnswer));
    }
}